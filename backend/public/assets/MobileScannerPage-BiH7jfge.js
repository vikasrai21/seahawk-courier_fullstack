import{j as e}from"./page-landing-CREvANXP.js";import{r as s}from"./vendor-helmet-Dwc3L0SQ.js";import{l as Gn}from"./index-DQpQqxPI.js";import{a as ve}from"./page-import-CFGNhKM8.js";import{c as qn,n as Vn}from"./barcodeEngine-D60-g-s-.js";import{c as Hn,u as Qn}from"./vendor-react-DrB23wtn.js";import{b as nn,R as xt,b1 as sn,Y as Xn,aW as rn,aX as bt,Z as an,aH as yt,J as Yn,b2 as Kn,ac as Jn,d as on,a7 as Zn,H as cn,b3 as ln,aQ as dn,b4 as es,b5 as ts,a4 as ns,a1 as un,p as ss,X as rs}from"./vendor-icons-DyYViWBt.js";import"./page-reconcile-BQT9N3SU.js";import"./page-rate-calc-CC-LHkCs.js";function Nn(c,p){var S,m;try{if(!c||!p)return null;const j=Number(c.videoWidth||0),O=Number(c.videoHeight||0);if(!j||!O)return null;const $=(S=c.getBoundingClientRect)==null?void 0:S.call(c),g=(m=p.getBoundingClientRect)==null?void 0:m.call(p);if(!$||!g)return null;const f=Number($.width||0),G=Number($.height||0);if(!f||!G)return null;const b=Math.max(f/j,G/O),z=j*b,k=O*b,u=(f-z)/2,w=(G-k)/2,ke=g.left-$.left,E=g.top-$.top,ne=g.right-$.left,Q=g.bottom-$.top,K=(ke-u)/b,ge=(E-w)/b,se=(ne-u)/b,o=(Q-w)/b,J=(xe,be,v)=>Math.max(be,Math.min(v,xe)),h=J(Math.min(K,se),0,j),T=J(Math.min(ge,o),0,O),A=J(Math.max(K,se),0,j),M=J(Math.max(ge,o),0,O),de=Math.max(0,A-h),q=Math.max(0,M-T);return!de||!q?null:{x:h,y:T,w:de,h:q}}catch{return null}}function pn(c=[]){if(!c.length)return"";const p=[];return c.includes("blur")&&p.push("hold steady"),c.includes("glare")&&p.push("reduce glare"),c.includes("angle")&&p.push("straighten angle"),c.includes("dark")&&p.push("add light"),c.includes("low_edge")&&p.push("fill frame"),p.length?`Improve capture: ${p.join(", ")}.`:""}function is(c,p){if(!c||!p||!c.videoWidth||!c.videoHeight)return null;const S=Nn(c,p);if(!S)return null;const m=Math.max(0,Math.floor(S.x)),j=Math.max(0,Math.floor(S.y)),O=Math.max(24,Math.floor(S.w)),$=Math.max(24,Math.floor(S.h)),g=128,f=96,G=document.createElement("canvas");G.width=g,G.height=f;const b=G.getContext("2d",{willReadFrequently:!0});if(!b)return null;b.drawImage(c,m,j,Math.min(O,c.videoWidth-m),Math.min($,c.videoHeight-j),0,0,g,f);const z=b.getImageData(0,0,g,f).data,k=g*f,u=new Float32Array(k);let w=0,ke=0,E=0;for(let P=0,X=0;P<z.length;P+=4,X+=1){const y=.2126*z[P]+.7152*z[P+1]+.0722*z[P+2];u[X]=y,w+=y,y>=245&&(ke+=1),y<=24&&(E+=1)}let ne=0,Q=0,K=0,ge=0,se=0,o=0;const J=Math.max(4,Math.floor(f*.15)),h=Math.max(4,Math.floor(g*.15)),T=g;for(let P=1;P<f-1;P+=1)for(let X=1;X<g-1;X+=1){const y=P*T+X,ze=u[y],B=u[y-1],Ee=u[y+1],Re=u[y-T],ye=u[y+T],rt=Math.abs(Ee-B),it=Math.abs(ye-Re),we=rt+it,Te=Math.abs(4*ze-B-Ee-Re-ye);ne+=Te,we>58&&(Q+=1),P<=J&&(K+=we),P>=f-J&&(ge+=we),X<=h&&(se+=we),X>=g-h&&(o+=we)}const A=Math.max(1,(g-2)*(f-2)),M=w/k,de=ne/A,q=Q/A,xe=ke/k,be=E/k,v=Math.abs(K-ge)/Math.max(1,K+ge),_e=Math.abs(se-o)/Math.max(1,se+o),je=Math.max(v,_e),Z=[];return de<22&&Z.push("blur"),xe>.18&&Z.push("glare"),(be>.55||M<40)&&Z.push("dark"),q<.08&&Z.push("low_edge"),je>.62&&Z.push("angle"),{ok:Z.length===0,issues:Z,metrics:{brightness:Number(M.toFixed(1)),blurScore:Number(de.toFixed(1)),glareRatio:Number((xe*100).toFixed(1)),edgeRatio:Number((q*100).toFixed(1)),perspectiveSkew:Number((je*100).toFixed(1))}}}function nt(c,p){const S=Number(c);return Number.isFinite(S)&&S>0?S:p}function as({samples:c=[],awb:p,now:S=Date.now(),stabilityWindowMs:m=1100,requiredHits:j=3}){const O=nt(m,1100),$=Math.max(1,Math.floor(nt(j,3))),g=nt(S,Date.now()),f=String(p||"").trim(),G=Array.isArray(c)?c.filter(k=>(k==null?void 0:k.awb)&&g-((k==null?void 0:k.at)||0)<=O):[];if(!f)return{samples:G,hits:0,isStable:!1};const b=[...G,{awb:f,at:g}],z=b.reduce((k,u)=>u.awb===f?k+1:k,0);return{samples:b,hits:z,isStable:z>=$}}function os({currentAttempts:c=0,maxReframeAttempts:p=2}){const S=Math.max(0,Math.floor(nt(p,2))),m=Math.max(0,Math.floor(Number(c)||0))+1;return m<=S?{action:"reframe",attempts:m}:{action:"switch_to_document",attempts:S}}const cs=window.location.origin,mn={w:"90vw",h:"18vw"},et={w:"92vw",h:"130vw"},hn=3500,fn=900,ls=1e4,ds="mobile_scanner_offline_queue",gn="mobile_scanner_workflow_mode",xn="mobile_scanner_device_profile",us=80,ps=500,ms=1,bn=100,tt=2,wt=2,hs=500,yn=960,Ne=.68,fs=900,H={phone:"phone-camera",rugged:"rugged-scanner"},i={IDLE:"IDLE",SCANNING:"SCANNING",CAPTURING:"CAPTURING",PREVIEW:"PREVIEW",PROCESSING:"PROCESSING",REVIEWING:"REVIEWING",APPROVING:"APPROVING",SUCCESS:"SUCCESS",ERROR:"ERROR"},gs=c=>{var p;try{(p=navigator==null?void 0:navigator.vibrate)==null||p.call(navigator,c)}catch{}},wn={tap:[20],lock:[100,40,100],success:[18,28,72],warning:[70,50,70],retry:[28,40,28],error:[110,55,110],duplicate:[90,50,90,50,90]},C=(c="tap")=>{gs(wn[c]||wn.tap)},st=(c,p,S="sine")=>{try{const m=new(window.AudioContext||window.webkitAudioContext),j=m.createOscillator(),O=m.createGain();j.type=S,j.frequency.setValueAtTime(c,m.currentTime),O.gain.setValueAtTime(.12,m.currentTime),O.gain.exponentialRampToValueAtTime(.01,m.currentTime+p),j.connect(O),O.connect(m.destination),j.start(),j.stop(m.currentTime+p)}catch{}},oe=()=>{st(880,.12),setTimeout(()=>st(1100,.1),130)},xs=()=>st(600,.08),te=()=>st(200,.25,"sawtooth"),Cn=c=>{try{if(!window.speechSynthesis)return;window.speechSynthesis.cancel();const p=new SpeechSynthesisUtterance(c);p.rate=1.2,p.pitch=1,p.lang="en-IN",window.speechSynthesis.speak(p)}catch{}},Sn=()=>{var c;try{if(typeof window>"u")return!1;if(window.isSecureContext)return!0;const p=((c=window.location)==null?void 0:c.hostname)||"";return p==="localhost"||p==="127.0.0.1"}catch{return!1}},r={bg:"#FAFBFD",surface:"#FFFFFF",border:"rgba(0,0,0,0.06)",text:"#111827",muted:"#6B7280",mutedLight:"#9CA3AF",primary:"#4F46E5",primaryLight:"#EEF2FF",success:"#059669",successLight:"#ECFDF5",warning:"#D97706",warningLight:"#FFFBEB",error:"#DC2626",errorLight:"#FEF2F2"},bs=`
