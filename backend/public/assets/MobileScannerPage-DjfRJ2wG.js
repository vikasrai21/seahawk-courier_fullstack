import{j as e}from"./page-landing-CREvANXP.js";import{r as s}from"./vendor-helmet-Dwc3L0SQ.js";import{l as qn}from"./index-bNapJ6HQ.js";import{a as ve}from"./page-import-CFGNhKM8.js";import{c as Vn,n as Hn}from"./barcodeEngine-CADh5QA1.js";import{c as Qn,u as Yn}from"./vendor-react-DrB23wtn.js";import{b as nn,R as xt,b1 as sn,Y as Kn,aW as rn,aX as bt,Z as on,aH as yt,J as Xn,b2 as Jn,ac as Zn,d as an,a7 as es,H as cn,b3 as ln,aQ as dn,b4 as ts,b5 as ns,a4 as ss,a1 as un,p as rs,X as is}from"./vendor-icons-DyYViWBt.js";import"./page-reconcile-BQT9N3SU.js";import"./page-rate-calc-CC-LHkCs.js";function jn(c,p){var S,m;try{if(!c||!p)return null;const k=Number(c.videoWidth||0),B=Number(c.videoHeight||0);if(!k||!B)return null;const $=(S=c.getBoundingClientRect)==null?void 0:S.call(c),f=(m=p.getBoundingClientRect)==null?void 0:m.call(p);if(!$||!f)return null;const g=Number($.width||0),G=Number($.height||0);if(!g||!G)return null;const b=Math.max(g/k,G/B),z=k*b,j=B*b,u=(g-z)/2,w=(G-j)/2,je=f.left-$.left,E=f.top-$.top,ne=f.right-$.left,Q=f.bottom-$.top,X=(je-u)/b,fe=(E-w)/b,se=(ne-u)/b,a=(Q-w)/b,J=(xe,be,v)=>Math.max(be,Math.min(v,xe)),h=J(Math.min(X,se),0,k),T=J(Math.min(fe,a),0,B),A=J(Math.max(X,se),0,k),M=J(Math.max(fe,a),0,B),le=Math.max(0,A-h),q=Math.max(0,M-T);return!le||!q?null:{x:h,y:T,w:le,h:q}}catch{return null}}function pn(c=[]){if(!c.length)return"";const p=[];return c.includes("blur")&&p.push("hold steady"),c.includes("glare")&&p.push("reduce glare"),c.includes("angle")&&p.push("straighten angle"),c.includes("dark")&&p.push("add light"),c.includes("low_edge")&&p.push("fill frame"),p.length?`Improve capture: ${p.join(", ")}.`:""}function os(c,p){if(!c||!p||!c.videoWidth||!c.videoHeight)return null;const S=jn(c,p);if(!S)return null;const m=Math.max(0,Math.floor(S.x)),k=Math.max(0,Math.floor(S.y)),B=Math.max(24,Math.floor(S.w)),$=Math.max(24,Math.floor(S.h)),f=128,g=96,G=document.createElement("canvas");G.width=f,G.height=g;const b=G.getContext("2d",{willReadFrequently:!0});if(!b)return null;b.drawImage(c,m,k,Math.min(B,c.videoWidth-m),Math.min($,c.videoHeight-k),0,0,f,g);const z=b.getImageData(0,0,f,g).data,j=f*g,u=new Float32Array(j);let w=0,je=0,E=0;for(let P=0,Y=0;P<z.length;P+=4,Y+=1){const y=.2126*z[P]+.7152*z[P+1]+.0722*z[P+2];u[Y]=y,w+=y,y>=245&&(je+=1),y<=24&&(E+=1)}let ne=0,Q=0,X=0,fe=0,se=0,a=0;const J=Math.max(4,Math.floor(g*.15)),h=Math.max(4,Math.floor(f*.15)),T=f;for(let P=1;P<g-1;P+=1)for(let Y=1;Y<f-1;Y+=1){const y=P*T+Y,ze=u[y],O=u[y-1],Ee=u[y+1],Re=u[y-T],ye=u[y+T],rt=Math.abs(Ee-O),it=Math.abs(ye-Re),we=rt+it,Te=Math.abs(4*ze-O-Ee-Re-ye);ne+=Te,we>58&&(Q+=1),P<=J&&(X+=we),P>=g-J&&(fe+=we),Y<=h&&(se+=we),Y>=f-h&&(a+=we)}const A=Math.max(1,(f-2)*(g-2)),M=w/j,le=ne/A,q=Q/A,xe=je/j,be=E/j,v=Math.abs(X-fe)/Math.max(1,X+fe),_e=Math.abs(se-a)/Math.max(1,se+a),ke=Math.max(v,_e),Z=[];return le<22&&Z.push("blur"),xe>.18&&Z.push("glare"),(be>.55||M<40)&&Z.push("dark"),q<.08&&Z.push("low_edge"),ke>.62&&Z.push("angle"),{ok:Z.length===0,issues:Z,metrics:{brightness:Number(M.toFixed(1)),blurScore:Number(le.toFixed(1)),glareRatio:Number((xe*100).toFixed(1)),edgeRatio:Number((q*100).toFixed(1)),perspectiveSkew:Number((ke*100).toFixed(1))}}}function nt(c,p){const S=Number(c);return Number.isFinite(S)&&S>0?S:p}function as({samples:c=[],awb:p,now:S=Date.now(),stabilityWindowMs:m=1100,requiredHits:k=3}){const B=nt(m,1100),$=Math.max(1,Math.floor(nt(k,3))),f=nt(S,Date.now()),g=String(p||"").trim(),G=Array.isArray(c)?c.filter(j=>(j==null?void 0:j.awb)&&f-((j==null?void 0:j.at)||0)<=B):[];if(!g)return{samples:G,hits:0,isStable:!1};const b=[...G,{awb:g,at:f}],z=b.reduce((j,u)=>u.awb===g?j+1:j,0);return{samples:b,hits:z,isStable:z>=$}}function cs({currentAttempts:c=0,maxReframeAttempts:p=2}){const S=Math.max(0,Math.floor(nt(p,2))),m=Math.max(0,Math.floor(Number(c)||0))+1;return m<=S?{action:"reframe",attempts:m}:{action:"switch_to_document",attempts:S}}const ls=window.location.origin,mn={w:"90vw",h:"18vw"},et={w:"92vw",h:"130vw"},hn=3500,gn=900,ds=1e4,us="mobile_scanner_offline_queue",fn="mobile_scanner_workflow_mode",xn="mobile_scanner_device_profile",ps=80,ms=1500,hs=2,bn=100,tt=2,wt=2,gs=500,yn=960,Ne=.68,fs=900,H={phone:"phone-camera",rugged:"rugged-scanner"},i={IDLE:"IDLE",SCANNING:"SCANNING",CAPTURING:"CAPTURING",PREVIEW:"PREVIEW",PROCESSING:"PROCESSING",REVIEWING:"REVIEWING",APPROVING:"APPROVING",SUCCESS:"SUCCESS",ERROR:"ERROR"},xs=c=>{var p;try{(p=navigator==null?void 0:navigator.vibrate)==null||p.call(navigator,c)}catch{}},wn={tap:[20],lock:[24,24,24],success:[18,28,72],warning:[70,50,70],retry:[28,40,28],error:[110,55,110],duplicate:[90,50,90,50,90]},C=(c="tap")=>{xs(wn[c]||wn.tap)},st=(c,p,S="sine")=>{try{const m=new(window.AudioContext||window.webkitAudioContext),k=m.createOscillator(),B=m.createGain();k.type=S,k.frequency.setValueAtTime(c,m.currentTime),B.gain.setValueAtTime(.12,m.currentTime),B.gain.exponentialRampToValueAtTime(.01,m.currentTime+p),k.connect(B),B.connect(m.destination),k.start(),k.stop(m.currentTime+p)}catch{}},ce=()=>{st(880,.12),setTimeout(()=>st(1100,.1),130)},Cn=()=>st(600,.08),te=()=>st(200,.25,"sawtooth"),Sn=c=>{try{if(!window.speechSynthesis)return;window.speechSynthesis.cancel();const p=new SpeechSynthesisUtterance(c);p.rate=1.2,p.pitch=1,p.lang="en-IN",window.speechSynthesis.speak(p)}catch{}},vn=()=>{var c;try{if(typeof window>"u")return!1;if(window.isSecureContext)return!0;const p=((c=window.location)==null?void 0:c.hostname)||"";return p==="localhost"||p==="127.0.0.1"}catch{return!1}},r={bg:"#FAFBFD",surface:"#FFFFFF",border:"rgba(0,0,0,0.06)",text:"#111827",muted:"#6B7280",mutedLight:"#9CA3AF",primary:"#4F46E5",primaryLight:"#EEF2FF",success:"#059669",successLight:"#ECFDF5",warning:"#D97706",warningLight:"#FFFBEB",error:"#DC2626",errorLight:"#FEF2F2"},bs=`
