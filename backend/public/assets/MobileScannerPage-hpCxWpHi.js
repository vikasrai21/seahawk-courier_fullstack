import{j as e}from"./page-landing-CREvANXP.js";import{r as s}from"./vendor-helmet-Dwc3L0SQ.js";import{l as qn}from"./index-Bhh5uyyL.js";import{a as ve}from"./page-import-CFGNhKM8.js";import{c as Vn,n as Hn}from"./barcodeEngine-B5KdHMFy.js";import{c as Qn,u as Yn}from"./vendor-react-DrB23wtn.js";import{b as nn,R as xt,b1 as sn,Y as Kn,aW as rn,aX as bt,Z as on,aH as yt,J as Xn,b2 as Jn,ac as Zn,d as an,a7 as es,H as cn,b3 as ln,aQ as dn,b4 as ts,b5 as ns,a4 as ss,a1 as un,p as is,X as rs}from"./vendor-icons-DyYViWBt.js";import"./page-reconcile-BQT9N3SU.js";import"./page-rate-calc-CC-LHkCs.js";function jn(c,p){var S,m;try{if(!c||!p)return null;const k=Number(c.videoWidth||0),W=Number(c.videoHeight||0);if(!k||!W)return null;const $=(S=c.getBoundingClientRect)==null?void 0:S.call(c),f=(m=p.getBoundingClientRect)==null?void 0:m.call(p);if(!$||!f)return null;const g=Number($.width||0),G=Number($.height||0);if(!g||!G)return null;const b=Math.max(g/k,G/W),B=k*b,j=W*b,u=(g-B)/2,w=(G-j)/2,je=f.left-$.left,E=f.top-$.top,ne=f.right-$.left,Q=f.bottom-$.top,X=(je-u)/b,ge=(E-w)/b,se=(ne-u)/b,a=(Q-w)/b,J=(fe,xe,v)=>Math.max(xe,Math.min(v,fe)),h=J(Math.min(X,se),0,k),z=J(Math.min(ge,a),0,W),A=J(Math.max(X,se),0,k),T=J(Math.max(ge,a),0,W),le=Math.max(0,A-h),q=Math.max(0,T-z);return!le||!q?null:{x:h,y:z,w:le,h:q}}catch{return null}}function pn(c=[]){if(!c.length)return"";const p=[];return c.includes("blur")&&p.push("hold steady"),c.includes("glare")&&p.push("reduce glare"),c.includes("angle")&&p.push("straighten angle"),c.includes("dark")&&p.push("add light"),c.includes("low_edge")&&p.push("fill frame"),p.length?`Improve capture: ${p.join(", ")}.`:""}function os(c,p){if(!c||!p||!c.videoWidth||!c.videoHeight)return null;const S=jn(c,p);if(!S)return null;const m=Math.max(0,Math.floor(S.x)),k=Math.max(0,Math.floor(S.y)),W=Math.max(24,Math.floor(S.w)),$=Math.max(24,Math.floor(S.h)),f=128,g=96,G=document.createElement("canvas");G.width=f,G.height=g;const b=G.getContext("2d",{willReadFrequently:!0});if(!b)return null;b.drawImage(c,m,k,Math.min(W,c.videoWidth-m),Math.min($,c.videoHeight-k),0,0,f,g);const B=b.getImageData(0,0,f,g).data,j=f*g,u=new Float32Array(j);let w=0,je=0,E=0;for(let L=0,Y=0;L<B.length;L+=4,Y+=1){const y=.2126*B[L]+.7152*B[L+1]+.0722*B[L+2];u[Y]=y,w+=y,y>=245&&(je+=1),y<=24&&(E+=1)}let ne=0,Q=0,X=0,ge=0,se=0,a=0;const J=Math.max(4,Math.floor(g*.15)),h=Math.max(4,Math.floor(f*.15)),z=f;for(let L=1;L<g-1;L+=1)for(let Y=1;Y<f-1;Y+=1){const y=L*z+Y,ze=u[y],O=u[y-1],Ee=u[y+1],Re=u[y-z],be=u[y+z],it=Math.abs(Ee-O),rt=Math.abs(be-Re),ye=it+rt,Te=Math.abs(4*ze-O-Ee-Re-be);ne+=Te,ye>58&&(Q+=1),L<=J&&(X+=ye),L>=g-J&&(ge+=ye),Y<=h&&(se+=ye),Y>=f-h&&(a+=ye)}const A=Math.max(1,(f-2)*(g-2)),T=w/j,le=ne/A,q=Q/A,fe=je/j,xe=E/j,v=Math.abs(X-ge)/Math.max(1,X+ge),_e=Math.abs(se-a)/Math.max(1,se+a),ke=Math.max(v,_e),Z=[];return le<22&&Z.push("blur"),fe>.18&&Z.push("glare"),(xe>.55||T<40)&&Z.push("dark"),q<.08&&Z.push("low_edge"),ke>.62&&Z.push("angle"),{ok:Z.length===0,issues:Z,metrics:{brightness:Number(T.toFixed(1)),blurScore:Number(le.toFixed(1)),glareRatio:Number((fe*100).toFixed(1)),edgeRatio:Number((q*100).toFixed(1)),perspectiveSkew:Number((ke*100).toFixed(1))}}}function nt(c,p){const S=Number(c);return Number.isFinite(S)&&S>0?S:p}function as({samples:c=[],awb:p,now:S=Date.now(),stabilityWindowMs:m=1100,requiredHits:k=3}){const W=nt(m,1100),$=Math.max(1,Math.floor(nt(k,3))),f=nt(S,Date.now()),g=String(p||"").trim(),G=Array.isArray(c)?c.filter(j=>(j==null?void 0:j.awb)&&f-((j==null?void 0:j.at)||0)<=W):[];if(!g)return{samples:G,hits:0,isStable:!1};const b=[...G,{awb:g,at:f}],B=b.reduce((j,u)=>u.awb===g?j+1:j,0);return{samples:b,hits:B,isStable:B>=$}}function cs({currentAttempts:c=0,maxReframeAttempts:p=2}){const S=Math.max(0,Math.floor(nt(p,2))),m=Math.max(0,Math.floor(Number(c)||0))+1;return m<=S?{action:"reframe",attempts:m}:{action:"switch_to_document",attempts:S}}const ls=window.location.origin,mn={w:"90vw",h:"18vw"},et={w:"92vw",h:"130vw"},hn=3500,gn=900,ds=1e4,us="mobile_scanner_offline_queue",fn="mobile_scanner_workflow_mode",xn="mobile_scanner_device_profile",ps=80,ms=1500,hs=2,bn=100,tt=2,wt=2,gs=500,yn=960,Ne=.68,fs=900,H={phone:"phone-camera",rugged:"rugged-scanner"},o={IDLE:"IDLE",SCANNING:"SCANNING",CAPTURING:"CAPTURING",PREVIEW:"PREVIEW",PROCESSING:"PROCESSING",REVIEWING:"REVIEWING",APPROVING:"APPROVING",SUCCESS:"SUCCESS",ERROR:"ERROR"},xs=c=>{var p;try{(p=navigator==null?void 0:navigator.vibrate)==null||p.call(navigator,c)}catch{}},wn={tap:[20],lock:[24,24,24],success:[18,28,72],warning:[70,50,70],retry:[28,40,28],error:[110,55,110],duplicate:[90,50,90,50,90]},C=(c="tap")=>{xs(wn[c]||wn.tap)},st=(c,p,S="sine")=>{try{const m=new(window.AudioContext||window.webkitAudioContext),k=m.createOscillator(),W=m.createGain();k.type=S,k.frequency.setValueAtTime(c,m.currentTime),W.gain.setValueAtTime(.12,m.currentTime),W.gain.exponentialRampToValueAtTime(.01,m.currentTime+p),k.connect(W),W.connect(m.destination),k.start(),k.stop(m.currentTime+p)}catch{}},ce=()=>{st(880,.12),setTimeout(()=>st(1100,.1),130)},Cn=()=>st(600,.08),te=()=>st(200,.25,"sawtooth"),Sn=c=>{try{if(!window.speechSynthesis)return;window.speechSynthesis.cancel();const p=new SpeechSynthesisUtterance(c);p.rate=1.2,p.pitch=1,p.lang="en-IN",window.speechSynthesis.speak(p)}catch{}},vn=()=>{var c;try{if(typeof window>"u")return!1;if(window.isSecureContext)return!0;const p=((c=window.location)==null?void 0:c.hostname)||"";return p==="localhost"||p==="127.0.0.1"}catch{return!1}},i={bg:"#FAFBFD",surface:"#FFFFFF",border:"rgba(0,0,0,0.06)",text:"#111827",muted:"#6B7280",mutedLight:"#9CA3AF",primary:"#4F46E5",primaryLight:"#EEF2FF",success:"#059669",successLight:"#ECFDF5",warning:"#D97706",warningLight:"#FFFBEB",error:"#DC2626",errorLight:"#FEF2F2"},bs=`
