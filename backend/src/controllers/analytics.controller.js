...
async function overview(req, res) {
  try {
    ...
    const result = await cache.wrap(key, async () => {
      ...
      return {
        kpis: { totalShipments, delivered, rto, ndr, deliveryRate: `${deliveryRate}%`, rtoRate: `${rtoRate}%`, totalRevenue: isImport ? revenue.amount : (revenue._sum.amount || 0), totalWeight: isImport ? revenue.weight : (revenue._sum.weight || 0) },
        byStatus:  _mapGroupBy(byStatus,  'status',  'count'),
        byCourier: _mapGroupBy(byCourier, 'courier', 'count'),
        courierRevenue: byCourier.map(c => ({ courier: c.courier ?? c.value, revenue: isImport ? (c.amount || 0) : (c._sum.amount || 0), count: isImport ? c.count : c._count.id })),
  ...
  try {
    ...
    const result = await cache.wrap(key, async () => {
      ...
      for (let m = 1; m <= 12; m++) {
        ...
        months.push({ month: m, name: new Date(year, m-1, 1).toLocaleString('en-IN', { month: 'short' }), count, revenue: isImport ? revenue.amount : (revenue._sum.amount || 0), deliveryRate: count > 0 ? parseFloat(((delivered/count)*100).toFixed(1)) : 0 });
      }
