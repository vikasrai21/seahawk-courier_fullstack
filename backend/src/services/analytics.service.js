...
async function _getDailyTrend(dateFilter = {}) {
  const where = dateFilter?.date
    ...
    : { date: { gte: new Date(Date.now() - 30 * 86400000).toISOString().split('T')[0] } };
...
async function getOverview(dateFrom, dateTo) {
  ...
  const [totalShipments, delivered, rto, ndr, revenue, byStatus, byCourier, recentTrend] =
    await Promise.all([
  ...
  const deliveryRate = totalShipments > 0 ? ((delivered / totalShipments) * 100).toFixed(1) : '0.0';
  const rtoRate = totalShipments > 0 ? ((rto / totalShipments) * 100).toFixed(1) : '0.0';
