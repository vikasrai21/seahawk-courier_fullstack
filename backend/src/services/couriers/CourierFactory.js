'use strict';
// src/services/couriers/CourierFactory.js
// Clean abstraction layer — add new couriers by creating a new Provider file
// and registering it here. Nothing else changes.

const logger = require('../../utils/logger');

// ─────────────────────────────────────────────────────────────
// BASE PROVIDER — every courier must implement these methods
// ─────────────────────────────────────────────────────────────
...
class DelhiveryProvider extends ICourierProvider {
  ...
  async createShipment(payload) {
    const {
      awb, consignee, phone, deliveryAddress, deliveryCity,
      deliveryState, pincode, weight, codAmount, productName,
    } = payload;
    ...
    return {
      awb,
      labelUrl:          `${this.baseUrl}/api/p/packing_slip?wbns=${awb}&pdf=true`,
  ...
  async trackShipment(awb) {
    const res = await fetch(
      `${this.baseUrl}/api/v1/packages/json/?waybill=${awb}&verbose=1`,
  ...
  async getLabel(awb) {
    return { url: `${this.baseUrl}/api/p/packing_slip?wbns=${awb}&pdf=true`, type: 'url' };
  }

  async calculateRate({ originPin, destPin, weight, cod }) {
    const params = new URLSearchParams({
      md: 'S', ss: 'Delivered',
      d_pin: destPin, o_pin: originPin,
      cgm: Math.round((weight || 0.5) * 1000),
      pt:  cod ? 'COD' : 'Pre-paid',
      cod: cod || 0,
    });
    const res = await fetch(
      `${this.baseUrl}/api/kinko/v1/invoice/charges/?${params}`,
      { headers: this._headers(), signal: AbortSignal.timeout(10000) }
    );
    return res.json();
  }
  ...
  async checkServiceability(originPin, destPin) {
    const res = await fetch(
      `${this.baseUrl}/c/api/pin-codes/json/?filter_codes=${destPin}`,
      { headers: this._headers(), signal: AbortSignal.timeout(10000) }
    );
    const data = await res.json();
    const serviceable = data?.delivery_codes?.length > 0;
    return { courier: 'Delhivery', serviceable, destPin };
  }
...
class DTDCProvider extends ICourierProvider {
  ...
  get customerCode() { return process.env.DTDC_CUSTOMER_CODE; }

  _headers() {
    return {
      'APPKEY':       process.env.DTDC_API_KEY,
      'Content-Type': 'application/json',
    };
  }
  ...
  async createShipment(payload) {
    ...
    const body = {
      cnno:        awb,
      refno:       awb,
      custId:      this.customerCode,
      noOfPcs:     1,
      shipType:    codAmount > 0 ? 'COD' : 'Non-document',
      grossWt:     weight || 0.5,
      CodAmt:      codAmount || 0,
      collectAmt:  codAmount || 0,
      docketAmt:   0,
      packType:    'CARTON',
      invoiceNo:   awb,
      invoiceAmt:  0,
      consigneeDetails: {
        cneeName:    consignee,
        cneeAdd1:    deliveryAddress,
        cneeCity:    deliveryCity,
        cneeState:   deliveryState,
        cneePincode: String(pincode),
        cneeMobile:  phone,
      },
    };
  ...
  async trackShipment(awb) {
    const res = await fetch(
      `${this.baseUrl}/dtdcConnectRestApi/api/v1/trackSingle/${awb}`,
      { headers: this._headers(), signal: AbortSignal.timeout(10000) }
    );
    ...
    return {
      ...
      events: (data?.trackDetails || []).map(e => ({
        status:    e.status || '',
        location:  e.location || '',
        timestamp: e.dateTime ? new Date(e.dateTime) : new Date(),
      })),
    };
  }

  async cancelShipment(awb) {
    // DTDC cancellation via their API
    const res = await fetch(`${this.baseUrl}/dtdcConnectRestApi/api/v1/cancelShipment/${awb}`, {
      method:  'DELETE',
      headers: this._headers(),
      signal:  AbortSignal.timeout(10000),
    });
  ...
  async getLabel(awb) {
    return {
      url:  `${this.baseUrl}/dtdcConnectRestApi/api/v1/getLabel/${awb}`,
      type: 'url',
    };
  }
  ...
  async calculateRate({ originPin, destPin, weight, cod }) {
    const res = await fetch(`${this.baseUrl}/dtdcConnectRestApi/api/v1/calculateRate`, {
      method:  'POST',
      headers: this._headers(),
      body:    JSON.stringify({ custId: this.customerCode, pinCodeFrom: originPin, pinCodeTo: destPin, weight: weight || 0.5, codAmount: cod || 0 }),
      signal:  AbortSignal.timeout(10000),
    });
    return res.json();
  }
  ...
  async checkServiceability(originPin, destPin) {
    const res = await fetch(
      `${this.baseUrl}/dtdcConnectRestApi/api/v1/checkServiceability?pinCodeFrom=${originPin}&pinCodeTo=${destPin}`,
      { headers: this._headers(), signal: AbortSignal.timeout(10000) }
    );
    const data = await res.json();
...
class BlueDartProvider extends ICourierProvider {
  ...
  async createShipment(payload) {
    const {
      awb, consignee, phone, deliveryAddress, deliveryCity,
      pincode, weight, codAmount, productName,
    } = payload;
    ...
    const body = {
      ...
      Consignee: {
        ...
        ConsigneeAddress1: deliveryAddress,
        ConsigneeAddress2: deliveryCity,
      ...
      ShipmentDetails: {
        ...
        CollectableAmount: codAmount || 0,
        CommodityDescription: productName || 'Goods',
        Dimensions: [{ Breadth: 10, Height: 10, Length: 10, Count: 1 }],
      },
      Shipper: {
        OriginArea:   process.env.BLUEDART_AREA_CODE || 'Del',
        ShipperName:  'Sea Hawk Courier & Cargo',
        ShipperPhone: process.env.DELHIVERY_SELLER_PHONE || '9911565523',
      },
    };
  ...
  async trackShipment(awb) {
    ...
    return {
      status: scans[0]?.ScanCode || 'Unknown',
      events: scans.map(s => ({
        status:    s.ScanDescription || '',
        location:  s.ScannedLocation || '',
        timestamp: s.ScanDate ? new Date(s.ScanDate) : new Date(),
      })),
    };
  }

  async cancelShipment(awb) {
    return { success: false, message: 'BlueDart cancellation requires manual intervention. Call 1860-233-1234.' };
  }

  async getLabel(awb) {
    return { url: `${this.baseUrl}/in/transportation/waybill/v1/GenerateLabel?AWB=${awb}`, type: 'url' };
  }
...
// ─────────────────────────────────────────────────────────────
// COURIER FACTORY — the single interface for all couriers
// ─────────────────────────────────────────────────────────────
const PROVIDERS = {
...
class CourierFactory {
  ...
  static get(courierName) {
    const provider = PROVIDERS[courierName];
  ...
  /** List all registered couriers */
  static getAll() { return Object.keys(PROVIDERS); }

  /** List only couriers that have API keys configured */
  static getConfigured() {
    return Object.values(PROVIDERS).filter(p => p.enabled).map(p => p.name);
  }

  /** Check serviceability across all configured couriers */
  static async checkAllServiceability(originPin, destPin) {
    const results = await Promise.allSettled(
      Object.values(PROVIDERS)
        .filter(p => p.enabled)
        .map(p => p.checkServiceability(originPin, destPin))
    );
    return results
  ...
  static async compareRates(payload) {
    ...
    if (configured.length === 0) {
      // Return estimated rates from our internal rate engine when no APIs configured
    ...
    const apiRates = results
      .filter(r => r.status === 'fulfilled')
      .map(r => r.value);

    // Always include internal estimates for comparison
  ...
  /** Internal rate estimates when no API keys configured */
  static _internalRates({ weight = 0.5, cod = 0 }) {
    const w = parseFloat(weight) || 0.5;
    const codCharge = cod > 0 ? 40 : 0;
    return [
      { courier: 'Delhivery', rate: { total: Math.round((35 + Math.max(0, w - 0.5) * 25 + codCharge) * 1.18), currency: 'INR', estimated: true } },
      { courier: 'DTDC',      rate: { total: Math.round((40 + Math.max(0, w - 0.5) * 28 + codCharge) * 1.18), currency: 'INR', estimated: true } },
      { courier: 'BlueDart',  rate: { total: Math.round((55 + Math.max(0, w - 0.5) * 35 + codCharge) * 1.18), currency: 'INR', estimated: true } },
    ];
  }
}

module.exports = { CourierFactory, DelhiveryProvider, DTDCProvider, BlueDartProvider };
