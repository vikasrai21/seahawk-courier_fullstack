...
import { X, Maximize2, Minimize2, Send, Loader2, ChevronDown, Wallet, ShieldAlert, Package } from 'lucide-react';
...
export function SkyAIWidget() {
  ...
  const [messages, setMessages] = useState([
    { id: '1', role: 'model', content: "Hello admin. I'm SkyAI. I can analyze shipments, monitor margins, or check courier metrics. What do you need?", timestamp: new Date() }
  ]);
  ...
  const sendMessage = async (userMsg) => {
    ...
    try {
      ...
      const { data, error } = await api.post('/ops/assistant/chat', {
        message: userMsg,
        history
      });
  ...
  const getFollowups = (msg) => {
    ...
    switch (msg.actionType) {
      case 'TRACK_SHIPMENT':
        return ['Show pending NDRs', 'Courier performance', 'Track another AWB'];
      case 'LOOKUP_SHIPMENT':
        return ['Track this shipment', 'Show pending NDRs', 'Negative wallets'];
      case 'GET_PENDING_NDRS':
        return ['Track first AWB', 'Negative wallets', 'Courier performance'];
      case 'GET_WALLET_BALANCES':
        return ['Pending NDRs', 'Courier performance', 'Track AWB'];
      case 'CHECK_COURIER_PERFORMANCE':
        return ['Pending NDRs', 'Track AWB', 'Dashboard summary'];
      default:
        return ['Track AWB', 'Pending NDRs', 'Negative wallets'];
    }
  };
  ...
  const getFollowupIcon = (label) => {
    ...
    if (value.includes('courier')) return '↗';
    ...
    if (actionType === 'GET_WALLET_BALANCES') {
      return (
        <div style={{ background: dark ? 'rgba(0,0,0,0.2)' : '#f8fafc', borderRadius: 8, padding: 12, marginTop: 8, border: `1px solid ${dark ? 'rgba(255,255,255,0.05)' : '#e2e8f0'}` }}>
          ...
          {data.negativeBalances?.length === 0 ? <div style={{fontSize: 11}}>No negative balances!</div> : data.negativeBalances?.map(w => (
    ...
    if (actionType === 'TRACK_SHIPMENT' || actionType === 'LOOKUP_SHIPMENT') {
      ...
      return (
        <div style={{ background: dark ? 'rgba(0,0,0,0.2)' : '#f8fafc', borderRadius: 8, padding: 12, marginTop: 8, border: `1px solid ${dark ? 'rgba(255,255,255,0.05)' : '#e2e8f0'}` }}>
          ...
          {ship?.consignee && (
            <div style={{ fontSize: 11, color: dark ? '#94a3b8' : '#64748b', marginBottom: 8 }}>
              📦 {ship.consignee} → <span style={{ fontWeight: 600 }}>{ship.destination}</span>
            </div>
          )}
                ...
                <div key={idx} style={{ display: 'flex', gap: 8, fontSize: 11, padding: '4px 0', borderBottom: idx < track.events.length - 1 ? `1px solid ${dark ? 'rgba(255,255,255,0.03)' : '#f1f5f9'}` : 'none' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: dark ? '#64748b' : '#94a3b8' }}>
                      {ev.location} {ev.timestamp ? `• ${new Date(ev.timestamp).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}` : ''}
                    </div>
                  </div>
                </div>
              ))}
    ...
    <>
      ...
      {!isOpen && (
        ...
        >
          ...
          <div className="shk-avatar-float">
            <AssistantOrb size={60} />
          </div>
      ...
      {isOpen && (
        <div style={{
          ...
          background: dark ? 'linear-gradient(180deg, rgba(15,23,42,0.95) 0%, rgba(30,41,59,0.92) 100%)' : 'linear-gradient(180deg, rgba(255,255,255,0.92) 0%, rgba(248,250,252,0.96) 100%)',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 34, height: 34, background: 'rgba(255,255,255,0.2)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                <div className="shk-avatar-float">
                  <AssistantOrb size={34} compact />
                </div>
              </div>
              <div>
                <div style={{ fontWeight: 800, fontSize: 14 }}>SkyAI</div>
                <div style={{ fontSize: 10, opacity: 0.9 }}>Ops Command Copilot</div>
              </div>
            </div>
          ...
          <div style={{
                        ...
                        <button
                          ...
                          style={{
                            ...
                            boxShadow: dark
                              ...
                              : '0 8px 18px -14px rgba(99,102,241,0.45), inset 0 1px 0 rgba(255,255,255,0.95)',
                        ...
                        >
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                            <span style={{
                              ...
                              background: dark ? 'rgba(125,211,252,0.12)' : 'rgba(99,102,241,0.1)',
                              color: dark ? '#7dd3fc' : '#4f46e5',
                              fontSize: 9,
                              fontWeight: 900,
                              lineHeight: 1,
                            }}>{getFollowupIcon(label)}</span>
                            {label}
                          </span>
                        </button>
                      ))}
            ...
            {isLoading && (
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                ...
                <div style={{ fontSize: 12, color: dark ? '#94a3b8' : '#64748b' }}>Analyzing portal data...</div>
                <Loader2 size={14} className="shk-spin" style={{ color: '#a855f7' }} />
              </div>
            )}
          ...
          {/* Quick Suggestions */}
          {messages.length === 1 && (
            <div style={{ padding: '0 20px 12px', display: 'flex', gap: 8, overflowX: 'auto', background: dark ? '#0f172a' : '#f8fafc', className: 'shk-hide-scrollbar' }}>
      ...
      <style>{`
        ...
        .shk-hide-scrollbar::-webkit-scrollbar { display: none; }
        ...
        .shk-follow-chip:hover {
          ...
          box-shadow: ${dark ? "'inset 0 1px 0 rgba(255,255,255,0.06), 0 8px 18px -14px rgba(96,165,250,0.35)'" : "'0 12px 22px -16px rgba(79,70,229,0.34), inset 0 1px 0 rgba(255,255,255,0.98)'"};
          border-color: ${dark ? "'rgba(125,211,252,0.3)'" : "'rgba(99,102,241,0.28)'"};
        }
        .shk-follow-chip:active {
