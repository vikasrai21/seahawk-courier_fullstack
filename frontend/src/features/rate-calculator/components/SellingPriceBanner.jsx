      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <button onClick={() => setEditPrice(!editPrice)} className="bg-white/20 hover:bg-white/30 rounded-lg p-1 transition-all">
              <Edit3 className="w-3 h-3" />
            </button>
            {customPrice && (
              <button onClick={() => { setCustomPrice(''); setEditPrice(false); }} className="text-[10px] bg-white/20 hover:bg-red-500/50 px-2 py-0.5 rounded-lg">
                Reset
              </button>
            )}
          </div>
              ...
              {!customPrice && !activeContract && svcLevel === 'all' && zone && chargeWt > 0 && shipType !== 'air' && (() => {
                ...
                return economy && premium ? (
                  <div className="flex gap-2 mt-1.5 flex-wrap">
                    <span className="text-[10px] bg-white/15 rounded-lg px-2 py-1">💰 Economy: <strong>{fmt(economy.total)}</strong></span>
                    <span className="text-[10px] bg-white/15 rounded-lg px-2 py-1">⭐ Premium: <strong>{fmt(premium.total)}</strong></span>
        ...
        <div className="flex gap-2 shrink-0 flex-wrap">
          {['profit', 'cost'].map((mode) => (
            <button
              ...
              className={`px-3 py-2 rounded-xl text-xs font-bold transition-all ${sortMode === mode ? 'bg-white text-gray-800' : 'bg-white/20 hover:bg-white/30 text-white'}`}
            >
