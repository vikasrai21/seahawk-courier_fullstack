...
export default function AnalyticsPage({ toast }) {
  ...
  const [density, setDensity] = useState(
    () => localStorage.getItem("sh-density") || "Normal",
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [r1, r2, r3, r4, r5, r6] = await Promise.all([
        ...
        api.get(`/analytics/cost?${params}`),
      ]);
      setOverview(r1.data);
      const couriersList = Array.isArray(r2.data)
        ? r2.data
        : r2.data?.couriers || r2.data?.carriers || [];
  ...
  const projectionData = useMemo(() => {
    ...
    const growth = prevMonth.revenue
      ? lastMonth.revenue / prevMonth.revenue
      : 1;
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <KPI
                ...
                value={kpis?.totalShipments?.toLocaleString() || "0"}
                icon={Package}
                accent="#3b82f6"
                sub={`${PERIODS.find((p) => p.days === period)?.label} performance`}
                dark={true}
              />
              <KPI
              ...
              <KPI
                ...
                value={`${kpis?.rtoRate || "0"}%`}
                icon={RotateCcw}
                accent="#ef4444"
                sub="Risk corridor monitoring"
                dark={true}
              />
              {isOwner && (
                <KPI
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 rounded-[32px] border border-slate-200/60 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl p-8 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 leading-none mb-1">
                        Performance Trends
                      </h3>
                      <p className="text-xs font-bold text-slate-500">
                        Monthly volume and revenue overview
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="rounded-[32px] border border-slate-200/60 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl p-8 shadow-sm relative overflow-hidden">
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie
                      data={Object.entries(overview?.byStatus || {}).map(
                      ...
                      animationDuration={1500}
                    >
                      {Object.entries(overview?.byStatus || {}).map((_, i) => (
                ...
                <div className="mt-6 grid grid-cols-2 gap-3">
                  {Object.entries(overview?.byStatus || {})
                    ...
                    .map(([status, count], i) => (
                      <div
                        key={status}
                        className="flex flex-col p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50"
                      >
                ...
                <div className="rounded-[32px] border border-slate-200/60 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl p-8 shadow-sm">
                  <div className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-500">
                        Delivery timeframe distribution
                      </p>
                    </div>
                  </div>
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
                      <Pie
                        data={[
                          {
                            name: "Same Day",
                            value: sla.buckets?.sameDay || 0,
                          },
                          {
                            name: "1-2 Days",
                            value: sla.buckets?.twoDays || 0,
                          },
                          {
                            name: "3-5 Days",
                            value:
                              (sla.buckets?.threeDays || 0) +
                              (sla.buckets?.fiveDays || 0),
                          },
                          {
                            name: "5-7 Days",
                            value: sla.buckets?.sevenDays || 0,
                          },
                          { name: "Overdue", value: sla.buckets?.overdue || 0 },
                        ].filter((d) => d.value > 0)}
                        dataKey="value"
                        nameKey="name"
                        innerRadius={65}
                        ...
                        stroke="none"
                      >
                        <Cell fill="#10b981" /> {/* Same Day - Green */}
                        <Cell fill="#3b82f6" /> {/* 1-2 Days - Blue */}
                        <Cell fill="#8b5cf6" /> {/* 3-5 Days - Purple */}
                        <Cell fill="#f59e0b" /> {/* 5-7 Days - Amber */}
                        <Cell fill="#ef4444" /> {/* Overdue - Red */}
                      </Pie>
                      <Tooltip content={<CustomTooltip />} />
                    </PieChart>
                  </ResponsiveContainer>
            ...
            {/* Cost Trend (Only for Owner) */}
            {isOwner && cost?.trend?.length > 0 && (
              <div className="rounded-[32px] border border-slate-200/60 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl p-8 shadow-sm">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-500">
                      Average billing cost over time
                    </p>
                  </div>
                </div>
                <ResponsiveContainer width="100%" height={240}>
                  <AreaChart data={cost.trend}>
                    <defs>
            ...
            <div className="rounded-[32px] border border-slate-200/60 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl p-8 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-500">
                      Carrier benchmarking and ranking
                    </p>
                  </div>
                </div>
              ...
              <div
                className={`table-shell overflow-visible ${density === "Compact" ? "density-compact" : ""}`}
              >
                <table className="tbl w-full border-collapse">
                  <thead className="table-head">
                    <tr>
                      ...
                      <th className="text-right p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        Volume
                      </th>
                      <th className="text-right p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        Success %
                      ...
                      return (
                        <tr key={c.carrier || i} className="table-row group">
                          ...
                          <td className="p-4 text-right">
                            <div className="text-sm font-black text-slate-800 dark:text-white tabular-nums">
                              {Number(c.total || c.count || 0).toLocaleString()}
                            </div>
                          ...
                          <td className="p-4 text-right">
                            <div
                              className={`text-sm font-black tabular-nums ${isHealthy ? "text-emerald-500" : "text-amber-500"}`}
                            >
                              {fmtPct(rate)}
                            </div>
                            <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                          ...
                          <td className="p-4 text-right">
                            <div className="text-sm font-black text-slate-700 dark:text-slate-300 tabular-nums">
                              {c.avgDeliveryDays != null
                                ? `${Number(c.avgDeliveryDays).toFixed(1)}d`
                                : "—"}
                            </div>
                            <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                              Lead Time
                          ...
                          {isOwner && (
                            <td className="p-4 text-right">
                              <div className="text-sm font-black text-blue-600 tabular-nums">
                                {fmt(c.revenue || c.totalRevenue || 0)}
                              </div>
                              <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                                Billed
                              </div>
                          ...
                          <td className="p-4 text-right">
                            <div
                              className={`w-2 h-2 rounded-full inline-block ${isHealthy ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]"}`}
                            />
                          </td>
                        </tr>
                      );
                ...
                <ResponsiveContainer width="100%" height={240}>
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      cursor={{ fill: "rgba(239,68,68,0.05)", radius: 4 }}
                    />
                    <Bar
                      dataKey="count"
                      fill="#ef4444"
